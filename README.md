# IA Templatizer 2.0 User & Developer Manual

---

## Overview

**IA Templatizer 2.0** is a robust command-line tool for batch-generating metadata CSV files for Internet Archive workflows. It applies a user-defined metadata template (JSON) to an input CSV, filling in missing fields, generating standardized identifiers, expanding repeatable fields, and validating metadata. The output is a new CSV formatted for compatibility with Internet Archive CLI tools and Python library.
